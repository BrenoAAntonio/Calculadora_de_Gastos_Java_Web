...
<head>
    ...
    <title>${despesa != null ? 'Editar Despesa' : 'Cadastro de Despesa'}</title>
    ...
    <div class="body-content">
        <div class="container main-content">
            <div class="glass-container">
                <h2>${despesa != null ? 'Editar Despesa' : 'Cadastrar Despesa'}</h2>
                <% if (request.getAttribute("erro") != null) { %>
                    <p class="error-message"><%= request.getAttribute("erro") %></p>
                <% } %>
                <form action="<%= request.getContextPath() %>/despesas?action=${despesa != null ? 'editar' : 'cadastrar'}" method="post">
                    <c:if test="${despesa != null}">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="valor" name="valor" value="${despesa.valor}" required>
                    </div>
                    <div class="form-group">
                        <label for="dataDespesa">Data da Despesa:</label>
                        <input type="date" id="dataDespesa" name="dataDespesa" value="${despesa.dataDespesa}" required>
                    </div>
                    <div class="form-group">
                        ...
                        <select id="categoriaId" name="categoriaId" required>
                            ...
                            <c:forEach var="categoria" items="${categorias}">
                                <option value="${categoria.id}" ${despesa.categoria.id == categoria.id ? 'selected' : ''}>${categoria.nome}</option>
                            </c:forEach>
                        </select>
                    </div>
                    <button type="submit">${despesa != null ? 'Atualizar Despesa' : 'Salvar Despesa'}</button>
                </form>
            </div>
        </div>
        <div class="sidebar">
